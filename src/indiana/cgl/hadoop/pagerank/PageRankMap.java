	...
	public void map(LongWritable key, Text value, Context context)
		throws IOException, InterruptedException {
			...
			if (rrd.targetUrlsList.size()<=0){
				...
				int size=rrd.targetUrlsList.size();
                for(int i=0;i<size;i++)
				{
					int node=rrd.targetUrlsList.get(i);
					sb.append("#");
					//x+="#";
					sb.append(node);
					//x+=node;
					double val=rrd.rankValue/size;
					context.write(new LongWritable(node), new Text(Double.toString(val)));
				}
